...
template <typename MessageT = Message> class Transport {
  ...
  public:
    inline Transport(const std::string &name, NetworkInterface *network_interface, bool continuous = false,
    ...
    // Send a message
    inline void send(const MessageT &message) { send_message(message); }

    // Set message for continuous broadcasting
    inline void set_broadcast_message(const MessageT &message) {
        broadcast_message_ = message;
        has_broadcast_message_ = true;
    }

    // Handle incoming message (for external routing)
    inline void handle_incoming_message(const std::string &message, const std::string &from_addr, uint16_t from_port) {
        MessageT msg;
